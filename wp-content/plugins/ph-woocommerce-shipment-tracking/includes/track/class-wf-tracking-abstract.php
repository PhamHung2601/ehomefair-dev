 ...
 */
abstract class PH_ShipmentTrackingAbstract {
	...
	public function get_shipment_info( ) {
		...
		// Get the Credentals Status of Live Tracking Carriers
		...
		if( isset( $this->tracking_data[ $shipping_service ]['api_url'] ) ) {
			...
			// Check for API URL and Credentials added or not before calling API
			if( '' != $api_url && isset($carrier_status[$shipping_service]) && $carrier_status[$shipping_service] ) {
	...
	private function load_tracking_data( ) {
		$this->tracking_data = Ph_Shipment_Tracking_Util::load_tracking_data();
	}
		...
		$tracking_info_api_obj_array = array();
 		...
 		if( '' != trim($api_uri) ) {
			foreach ( $shipment_ids as $shipment_id ) {
				...
				$tracking_info_api_obj->api_live_status		= $api_tracking->livestatus;

				if( ! empty($api_tracking->shipment_progress) ) {
					$tracking_info_api_obj->shipment_progress = $api_tracking->shipment_progress;
				}
				else {
					$tracking_info_api_obj->shipment_progress	= array();
				}
	...
	private function get_tracking_link( $tracking_url, $shipment_id, $shipping_postcode ) {
		...
		$tracking_link 	= '';
		if( '' != $tracking_url ){
			if( stripos($tracking_url, Ph_Shipment_Tracking_Util::TAG_SHIPMENT_ID ) ) {
				$tracking_link 	= $tracking_url;
				$tracking_link 	= str_replace( Ph_Shipment_Tracking_Util::TAG_SHIPMENT_ID, $shipment_id, $tracking_link );
			}
			else {
				$tracking_link 	= $tracking_url.$shipment_id;
			}
			
			if( stripos($tracking_link, Ph_Shipment_Tracking_Util::TAG_SHIPPING_POST_CODE ) ) {
				$shipping_postcode = preg_replace('/\s+/', '', $shipping_postcode);
				$tracking_link 	= str_replace( Ph_Shipment_Tracking_Util::TAG_SHIPPING_POST_CODE, $shipping_postcode, $tracking_link );
			}
		}
