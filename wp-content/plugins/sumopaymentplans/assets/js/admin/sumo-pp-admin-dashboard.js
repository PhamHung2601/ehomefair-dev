...
jQuery( function( $ ) {
    ...
    $( '.expected_payment_date' ).datetimepicker( {
        minDate : 0,
        changeMonth : true,
        changeYear : true,
        dateFormat : 'yy-mm-dd',
    ...
    $( document ).on( 'click', '._sumo_pp_payment-orders a.edit-installment-date', function( evt ) {
        evt.preventDefault() ;
    ...
    $( '.post-type-sumo_pp_payments' ).find( 'form' ).on( 'submit', function( evt ) {
        if ( 1 >= $( '._sumo_pp_payment-orders' ).find( '.expected_payment_date' ).length ) {
            return true ;
        }

        var static_time = 0, err = false ;
        $( '._sumo_pp_payment-orders' ).find( '.expected_payment_date' ).each( function() {
        ...
        if ( true === err ) {
            window.alert( sumo_pp_admin_dashboard.warning_dates_not_in_order )
            return false ;
        }
    } ) ;
