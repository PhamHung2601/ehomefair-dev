...
class SUMO_PP_Payment extends SUMO_PP_Abstract_Payment {
    ...
    public function update_as_paid_order( $paid_order_id ) {
        ...
        if ( 'my_account' === get_post_meta( $paid_order_id, "{$this->prefix}created_via", true ) ) {
            if ( $this->balance_payable_order_exists() ) {
                remove_action( 'woocommerce_order_status_changed', array( _sumo_pp()->order, 'update_payments' ), 20, 3 ) ;
                ...
                add_action( 'woocommerce_order_status_changed', array( _sumo_pp()->order, 'update_payments' ), 20, 3 ) ;
            }
    ...
    public function process_initial_payment( $note = array(), $approve_by_admin = false, $status_to_update = 'in_progress' ) {
        ...
        if ( 'auto' === $this->get_payment_mode() ) {
            $this->add_payment_note( __( 'Future payments will be charged as Automatic.', SUMO_PP_PLUGIN_TEXT_DOMAIN ), 'success', __( 'Automatic Payments Approved', SUMO_PP_PLUGIN_TEXT_DOMAIN ) ) ;
        }
    ...
    public function process_balance_payment( $balance_payable_order, $note = array(), $status_to_update = 'in_progress' ) {
        ...
        _sumo_pp()->mailer->send( 'payment_plan_success', $balance_payable_order->order_id, $this ) ;
    ...
    public function payment_complete( $balance_payable_order, $note = array() ) {
        ...
        if ( 'pay-in-deposit' === $this->get_payment_type() ) {
            _sumo_pp()->mailer->send( 'deposit_balance_payment_completed', $balance_payable_order->order_id, $this ) ;
        } else {
            _sumo_pp()->mailer->send( 'payment_plan_success', $balance_payable_order->order_id, $this ) ;
            _sumo_pp()->mailer->send( 'payment_plan_completed', $balance_payable_order->order_id, $this ) ;
        }
