...
abstract class SUMO_PP_Abstract_Payment {
    ...
    public function charge_shipping_during() {
        ...
        $initial_payment_order = wc_get_order( $this->get_initial_payment_order_id() ) ;

        if ( ! $initial_payment_order || sizeof( $initial_payment_order->get_items() ) > 1 ) {
            return 'initial-payment' ;
        }
    ...
    public function set_payment_mode( $mode, $add_note = true ) {
        ...
        $this->update_prop( 'payment_mode_switched', 'yes' ) ;

        if ( $add_note ) {
            $this->add_payment_note( sprintf( __( 'Payment mode switched to %s.', $this->text_domain ), ('auto' === $mode ? 'Automatic' : 'Manual' ) ), 'success', __( 'Payment Mode Switched', $this->text_domain ) ) ;
        }
        return true ;
    }
