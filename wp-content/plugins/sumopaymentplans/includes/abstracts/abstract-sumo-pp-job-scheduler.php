...
abstract class SUMO_PP_Abstract_Job_Scheduler {
    ...
    /**
     * @var int The Job Scheduler post ID. Each job identifier.
     */
    protected $job_id = 0 ;
    ...
    /**
     * @var array Post Type for Job Schedulers
     */
    protected $post_type = 'sumo_pp_cron_jobs' ;

    /**
     * @var WP_Query object Job Schedulers query
     */
    public $query ;
    ...
    public function init_job() {
        ...
        if( ! $this->exists() ) {
            $this->job_id = wp_insert_post( array(
                ...
                'post_author'   => 1 ,
                'post_title'    => __( 'Payment Job Schedulers' , SUMO_PP_PLUGIN_TEXT_DOMAIN ) ,
                    ) ) ;
    ...
    public function get_next_scheduled_job( $job_name , $display = false ) {
        ...
        if( ! $scheduled_timestamp = current( $scheduled ) ) {
            return false ;
        }

        if( $display ) {
            return _sumo_pp_get_date_to_display( $scheduled_timestamp ) ;
        }
