...
class SUMO_PP_Job_Scheduler extends SUMO_PP_Abstract_Job_Scheduler {
    ...
    public function get_schedules_per_day( $no_of_days, $times ) {
        ...
        for ( $n = 0 ; $n < $no_of_days ; $n ++ ) {
            for ( $t = 1 ; $t <= $times ; $t ++ ) {
                ...
                $day_start_time   = _sumo_pp_get_timestamp( date( 'Y-m-d 00:00:00', $time ) ) ;
                $day_end_time     = _sumo_pp_get_timestamp( date( 'Y-m-d 23:59:59', $time ) ) ;
    ...
    /**
     * Get available schedules before specific time
     * Ex. 
     * 1) if $days_count = array(4,3,2,1) && $total_days === 2 then get timestamp before $schedule_before for last 2 days
     * 2) if $days_count = array(3,2,1) && $total_days === 5 then get timestamp before $schedule_before for last 3 days
     * 
     * @return array()
     */
    public function get_schedules_before_time( $schedule_before, $days_count, $total_days ) {
        ...
        foreach ( $days_count as $day_count ) {
            ...
            if ( $total_days >= $day_count ) {
                $timestamp = absint( $schedule_before - (86400 * $day_count) ) ;

                if ( $timestamp >= _sumo_pp_get_timestamp( 0, 0, true ) ) {
    ...
    public function schedule_reminder( $balance_payable_order_id, $remind_before, $mail_template_id ) {
        ...
        $total_days_to_notify = ceil( ($remind_before_time - $remind_from_time) / 86400 ) ;
        ...
        if ( isset( $reminder_intervals[ 'times-per-day' ] ) ) {//Schedule by times per day
            ...
            $schedules_before_time = $this->get_schedules_before_time( $remind_before_time, $reminder_intervals[ 'no-of-days' ], $total_days_to_notify ) ;

            if ( $schedules_before_time ) {
                foreach ( $schedules_before_time as $notify_time ) {
                    if ( $this->create_job( $notify_time, 'notify_reminder', array(
                                'balance_payable_order_id' => absint( $balance_payable_order_id ),
                                'mail_template_id'         => $mail_template_id
                            ) )
    ...
    public function schedule_automatic_pay( $balance_payable_order_id, $payment_date ) {
        ...
        switch ( $next_eligible_status = _sumo_pp_get_next_eligible_payment_failed_status( $this->payment ) ) {
            case 'overdue':
