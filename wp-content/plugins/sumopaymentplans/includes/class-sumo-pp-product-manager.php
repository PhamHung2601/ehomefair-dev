...
class SUMO_PP_Product_Manager {
    ...
    public function init() {
        ...
        add_filter( 'woocommerce_loop_add_to_cart_args', array( $this, 'prevent_ajax_add_to_cart' ), 10, 2 ) ;
        add_filter( 'woocommerce_product_add_to_cart_url', array( $this, 'redirect_to_single_product' ), 10, 2 ) ;
    ...
    public function current_user_can_purchase() {
        ...
        return self::$current_user_can_purchase = _sumo_pp_user_can_purchase_payment( get_current_user_id() ) ;
    }
    ...
    public function get_props( $product, $user_id = null ) {
        ...
        $props[ 'payment_type' ]          = get_post_meta( $props[ 'product_id' ], SUMO_PP_PLUGIN_PREFIX . 'payment_type', true ) ;
        $props[ 'apply_global_settings' ] = 'yes' === get_post_meta( $props[ 'product_id' ], SUMO_PP_PLUGIN_PREFIX . 'apply_global_settings', true ) ;
        $props[ 'is_available' ]          = true ;

        $from = get_post_meta( $props[ 'product_id' ], SUMO_PP_PLUGIN_PREFIX . 'available_dates_from', true ) ;
        $to   = get_post_meta( $props[ 'product_id' ], SUMO_PP_PLUGIN_PREFIX . 'available_dates_to', true ) ;
        ...
        if ( 'pay-in-deposit' === $props[ 'payment_type' ] ) {
            $props[ 'deposit_type' ] = $props[ 'apply_global_settings' ] ? get_option( SUMO_PP_PLUGIN_PREFIX . 'deposit_type', 'pre-defined' ) : get_post_meta( $props[ 'product_id' ], SUMO_PP_PLUGIN_PREFIX . 'deposit_type', true ) ;

            if ( 'user-defined' === $props[ 'deposit_type' ] ) {
                $props[ 'user_defined_deposit_type' ] = $props[ 'apply_global_settings' ] ? 'percent-of-product-price' : get_post_meta( $props[ 'product_id' ], SUMO_PP_PLUGIN_PREFIX . 'user_defined_deposit_type', true ) ;

                if ( 'fixed-price' === $props[ 'user_defined_deposit_type' ] && ! $props[ 'apply_global_settings' ] ) {
                    $props[ 'min_user_defined_deposit_price' ] = floatval( get_post_meta( $props[ 'product_id' ], SUMO_PP_PLUGIN_PREFIX . 'min_user_defined_deposit_price', true ) ) ;
                    $props[ 'max_user_defined_deposit_price' ] = floatval( get_post_meta( $props[ 'product_id' ], SUMO_PP_PLUGIN_PREFIX . 'max_user_defined_deposit_price', true ) ) ;
                } else {
                    $props[ 'min_deposit' ] = $props[ 'apply_global_settings' ] ? floatval( get_option( SUMO_PP_PLUGIN_PREFIX . 'min_deposit', '0.01' ) ) : floatval( get_post_meta( $props[ 'product_id' ], SUMO_PP_PLUGIN_PREFIX . 'min_deposit', true ) ) ;
                    $props[ 'max_deposit' ] = $props[ 'apply_global_settings' ] ? floatval( get_option( SUMO_PP_PLUGIN_PREFIX . 'max_deposit', '99.99' ) ) : floatval( get_post_meta( $props[ 'product_id' ], SUMO_PP_PLUGIN_PREFIX . 'max_deposit', true ) ) ;
                }
            } else {
                $props[ 'deposit_price_type' ] = $props[ 'apply_global_settings' ] ? 'percent-of-product-price' : get_post_meta( $props[ 'product_id' ], SUMO_PP_PLUGIN_PREFIX . 'deposit_price_type', true ) ;

                if ( 'percent-of-product-price' === $props[ 'deposit_price_type' ] ) {
                    $props[ 'fixed_deposit_percent' ] = $props[ 'apply_global_settings' ] ? floatval( get_option( SUMO_PP_PLUGIN_PREFIX . 'fixed_deposit_percent', '50' ) ) : floatval( get_post_meta( $props[ 'product_id' ], SUMO_PP_PLUGIN_PREFIX . 'fixed_deposit_percent', true ) ) ;
                } else {
                    $props[ 'fixed_deposit_price' ] = $props[ 'apply_global_settings' ] ? floatval( get_option( SUMO_PP_PLUGIN_PREFIX . 'fixed_deposit_price', '0' ) ) : floatval( get_post_meta( $props[ 'product_id' ], SUMO_PP_PLUGIN_PREFIX . 'fixed_deposit_price', true ) ) ;
                }
            }
            if ( $props[ 'apply_global_settings' ] ) {
                ...
                $props[ 'pay_balance_type' ] = '' === get_post_meta( $props[ 'product_id' ], SUMO_PP_PLUGIN_PREFIX . 'pay_balance_type', true ) ? 'after' : get_post_meta( $props[ 'product_id' ], SUMO_PP_PLUGIN_PREFIX . 'pay_balance_type', true ) ;

                if ( 'after' === $props[ 'pay_balance_type' ] ) {
                    $props[ 'pay_balance_after' ] = '' === get_post_meta( $props[ 'product_id' ], SUMO_PP_PLUGIN_PREFIX . 'balance_payment_due', true ) ? absint( get_post_meta( $props[ 'product_id' ], SUMO_PP_PLUGIN_PREFIX . 'pay_balance_after', true ) ) : absint( get_post_meta( $props[ 'product_id' ], SUMO_PP_PLUGIN_PREFIX . 'balance_payment_due', true ) ) ;
                } else {
                    $props[ 'pay_balance_before' ]             = get_post_meta( $props[ 'product_id' ], SUMO_PP_PLUGIN_PREFIX . 'pay_balance_before', true ) ;
                    $props[ 'set_expired_deposit_payment_as' ] = get_post_meta( $props[ 'product_id' ], SUMO_PP_PLUGIN_PREFIX . 'set_expired_deposit_payment_as', true ) ;
                }
            }
        } else if ( 'payment-plans' === $props[ 'payment_type' ] ) {
            $props[ 'selected_plans' ] = $props[ 'apply_global_settings' ] ? get_option( SUMO_PP_PLUGIN_PREFIX . 'selected_plans', array() ) : get_post_meta( $props[ 'product_id' ], SUMO_PP_PLUGIN_PREFIX . 'selected_plans', true ) ;
        ...
        $props[ 'force_deposit' ]         = $props[ 'apply_global_settings' ] ? ('payment-plans' === $props[ 'payment_type' ] ? get_option( SUMO_PP_PLUGIN_PREFIX . 'force_payment_plan', 'no' ) : get_option( SUMO_PP_PLUGIN_PREFIX . 'force_deposit', 'no' )) : get_post_meta( $props[ 'product_id' ], SUMO_PP_PLUGIN_PREFIX . 'force_deposit', true ) ;
        $props[ 'apply_global_settings' ] = $props[ 'apply_global_settings' ] ? 'yes' : 'no' ;

        return self::$props = wp_parse_args( ( array ) apply_filters( 'sumopaymentplans_get_product_props', $props ), $this->get_default_props() ) ;
    }
    ...
    public function get_deposit_form( $props = null, $hide_if_variation = false, $class = '' ) {
        if ( is_null( $props ) ) {
            $props = self::$props ;
        }
        ...
        if ( is_product() ) {
            ...
            if (
                ...
                if ( isset( self::$props[ 'booking_payment_end_date' ] ) ) {
                    ...
                    return true ;
                } else if ( _sumo_pp_get_timestamp( $this->get_prop( 'pay_balance_before' ) ) <= _sumo_pp_get_timestamp( 0, 0, true ) ) {
                    return false ;
                }
            }
        }
