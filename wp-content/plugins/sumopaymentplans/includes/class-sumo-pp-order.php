...
class SUMO_PP_Order {
    ...
    public function get_payment_items() {
        ...
        if ( $this->is_payment_order() ) {
            ...
            foreach ( $order_items as $item ) {
                ...
                $product_id    = $item[ 'variation_id' ] > 0 ? $item[ 'variation_id' ] : $item[ 'product_id' ] ;
                $product_props = _sumo_pp()->product->get_props( $product_id, $this->order->get_customer_id() ) ;
    ...
    public function is_orderpp_created_via_multiple() {
        if ( 'yes' === get_post_meta( $this->order_id, 'is' . SUMO_PP_PLUGIN_PREFIX . 'orderpp', true ) ) {
            return get_post_meta( $this->order_id, SUMO_PP_PLUGIN_PREFIX . 'payment_data', true ) ;
        }
        return false ;
    }

    public function contains_payment_data() {
        if ( $payment_data = $this->is_orderpp_created_via_multiple() ) {
            return $payment_data ;
        }
