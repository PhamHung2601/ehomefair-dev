...
class SUMO_PP_Ajax {
    ...
    public static function get_wc_booking_deposit_fields() {
        ...
        $product_props        = _sumo_pp()->product->get_props( $_POST[ 'product' ] ) ;
    ...
    public static function checkout_orderpp() {
        ...
        WC()->session->__unset( SUMO_PP_PLUGIN_PREFIX . 'orderpp_enabled' ) ;
        WC()->session->__unset( SUMO_PP_PLUGIN_PREFIX . 'orderpp_deposited_amount' ) ;
        WC()->session->__unset( SUMO_PP_PLUGIN_PREFIX . 'orderpp_chosen_payment_plan' ) ;

        if ( 'yes' === $_POST[ 'enabled' ] ) {
            WC()->session->set( SUMO_PP_PLUGIN_PREFIX . 'orderpp_enabled', 'yes' ) ;

            switch ( wc_clean( $_POST[ 'payment_type' ] ) ) {
                case 'pay-in-deposit':
                    if ( isset( $_POST[ 'deposited_amount' ] ) ) {
                        WC()->session->set( SUMO_PP_PLUGIN_PREFIX . 'orderpp_deposited_amount', $_POST[ 'deposited_amount' ] ) ;
                    }
                    break ;
                case 'payment-plans':
                    if ( isset( $_POST[ 'chosen_payment_plan' ] ) ) {
                        WC()->session->set( SUMO_PP_PLUGIN_PREFIX . 'orderpp_chosen_payment_plan', $_POST[ 'chosen_payment_plan' ] ) ;
                    }
                    break ;
            }
        }
    ...
    public static function bulk_update_products() {
        ...
        if ( empty( $data[ 'get_product_select_type' ] ) ) {
            wp_send_json_error( array(
                'itemsCount'  => 0,
                'errorNotice' => __( 'Something went wrong while updating the products!!', SUMO_PP_PLUGIN_TEXT_DOMAIN ),
            ) ) ;
        }

        //Save the settings
        update_option( 'bulk' . SUMO_PP_PLUGIN_PREFIX . 'get_product_select_type', wc_clean( $data[ 'get_product_select_type' ] ) ) ;
        update_option( 'bulk' . SUMO_PP_PLUGIN_PREFIX . 'get_included_categories',  ! empty( $data[ 'get_included_categories' ] ) ? wc_clean( $data[ 'get_included_categories' ] ) : array()  ) ;
        update_option( 'bulk' . SUMO_PP_PLUGIN_PREFIX . 'get_excluded_categories',  ! empty( $data[ 'get_excluded_categories' ] ) ? wc_clean( $data[ 'get_excluded_categories' ] ) : array()  ) ;
        update_option( 'bulk' . SUMO_PP_PLUGIN_PREFIX . 'get_included_products',  ! empty( $data[ 'get_included_products' ] ) ? wc_clean( is_array( $data[ 'get_included_products' ] ) ? $data[ 'get_included_products' ] : explode( ',', $data[ 'get_included_products' ] )  ) : array()  ) ;
        update_option( 'bulk' . SUMO_PP_PLUGIN_PREFIX . 'get_excluded_products',  ! empty( $data[ 'get_excluded_products' ] ) ? wc_clean( is_array( $data[ 'get_excluded_products' ] ) ? $data[ 'get_excluded_products' ] : explode( ',', $data[ 'get_excluded_products' ] )  ) : array()  ) ;
        ...
        switch ( get_option( 'bulk' . SUMO_PP_PLUGIN_PREFIX . 'get_product_select_type' ) ) {
            ...
            case 'included-products':
                $found_products = array_map( 'absint', get_option( 'bulk' . SUMO_PP_PLUGIN_PREFIX . 'get_included_products', array() ) ) ;
                break ;
            case 'excluded-products':
                $products       = new WP_Query( array(
                    ...
                    'posts_per_page' => '-1',
                    'post_status'    => 'publish',
                    'post__not_in'   => array_map( 'absint', get_option( 'bulk' . SUMO_PP_PLUGIN_PREFIX . 'get_excluded_products', array() ) ),
                    'fields'         => 'ids',
                    'cache_results'  => false,
                    'tax_query'      => array(
                        array(
                ...
                $products = new WP_Query( array(
                    ...
                    'tax_query'      => array(
                        ...
                        array(
                            ...
                            'field'    => 'term_id',
                            'terms'    => array_map( 'absint', get_option( 'bulk' . SUMO_PP_PLUGIN_PREFIX . 'get_included_categories', array() ) ),
                            'operator' => 'IN',
                        ),
                        array(
                            'taxonomy' => 'product_type',
                    ...
                    'tax_query'      => array(
                        array(
                            ...
                            array(
                                'taxonomy' => 'product_cat',
                                'field'    => 'term_id',
                                'terms'    => array_map( 'absint', get_option( 'bulk' . SUMO_PP_PLUGIN_PREFIX . 'get_excluded_categories', array() ) ),
                                'operator' => 'NOT IN',
                            ),
                            array(
                                'taxonomy' => 'product_type',
        ...
        if ( empty( $found_products ) ) {
            wp_send_json_error( array(
                'itemsCount'  => 0,
                'errorNotice' => __( 'No products found to update.', SUMO_PP_PLUGIN_TEXT_DOMAIN ),
            ) ) ;
        }
        ...
        if ( ! $job_id || ! is_numeric( $job_id ) ) {
            wp_send_json_error( array(
                'itemsCount'  => sizeof( $found_products ),
                'errorNotice' => __( 'Something went wrong while updating the products!!', SUMO_PP_PLUGIN_TEXT_DOMAIN ),
            ) ) ;
        }
        ...
        wp_send_json_success( array(
            'itemsCount'    => sizeof( $found_products ),
            'successNotice' => sprintf( __( '%s product(s) found. Product is updating in the background. The Product update process may take a little while, so please be patient.', SUMO_PP_PLUGIN_TEXT_DOMAIN ), sizeof( $found_products ) ),
        ) ) ;
    }
