...
class SUMO_PP_Admin_Product {
    ...
    public static function get_product_settings() {
        ...
        $from = get_post_meta($post->ID, SUMO_PP_PLUGIN_PREFIX . 'available_dates_from', true) ;
        $to   = get_post_meta($post->ID, SUMO_PP_PLUGIN_PREFIX . 'available_dates_to', true) ;
        ?>
    ...
    public static function get_variation_product_settings( $loop , $variation_data , $variation ) {
        ...
        woocommerce_wp_select( array(
            ...
            'description'   => '<a href="#" class="'.SUMO_PP_PLUGIN_PREFIX.'schedule_availability'.$loop.'">' . __('Schedule', SUMO_PP_PLUGIN_TEXT_DOMAIN) . '</a>',
        )) ;

        $from = get_post_meta($variation->ID, SUMO_PP_PLUGIN_PREFIX . 'available_dates_from', true) ;
        $to   = get_post_meta($variation->ID, SUMO_PP_PLUGIN_PREFIX . 'available_dates_to', true) ;
        ?>
        <p class="form-field <?php echo SUMO_PP_PLUGIN_PREFIX . 'availability_dates_field' . ' ' . SUMO_PP_PLUGIN_PREFIX . "fields{$loop}" . ' ' . SUMO_PP_PLUGIN_PREFIX . "availability_dates_field{$loop}" ; ?>">
            <input type="text" class="short" name="<?php echo SUMO_PP_PLUGIN_PREFIX . "available_dates_from[{$loop}]" ; ?>" id="<?php echo SUMO_PP_PLUGIN_PREFIX . "available_dates_from{$loop}" ; ?>" value="<?php echo '' === $from ? '' : date_i18n( 'Y-m-d', _sumo_pp_get_timestamp( $from ) ) ?>" placeholder="<?php _e('From&hellip;YYYY-MM-DD', SUMO_PP_PLUGIN_TEXT_DOMAIN) ?>" maxlength="10" pattern="[0-9]{4}-(0[1-9]|1[012])-(0[1-9]|1[0-9]|2[0-9]|3[01])" />
            <input type="text" class="short" name="<?php echo SUMO_PP_PLUGIN_PREFIX . "available_dates_to[{$loop}]" ; ?>" id="<?php echo SUMO_PP_PLUGIN_PREFIX . "available_dates_to{$loop}" ; ?>" value="<?php echo '' === $to ? '' : date_i18n( 'Y-m-d', _sumo_pp_get_timestamp( $to ) ) ?>" placeholder="<?php _e('To&hellip;YYYY-MM-DD', SUMO_PP_PLUGIN_TEXT_DOMAIN) ?>" maxlength="10" pattern="[0-9]{4}-(0[1-9]|1[012])-(0[1-9]|1[0-9]|2[0-9]|3[01])" />
            <a href="#" class="description <?php echo SUMO_PP_PLUGIN_PREFIX . 'cancel_availability'.$loop ; ?>"><?php echo __('Cancel', SUMO_PP_PLUGIN_TEXT_DOMAIN) ?></a><?php echo wc_help_tip(__('The payment plan/deposit options will be given to users starting at 00:00:00 of "From" date and ending at 23:59:59 of "To" date.', SUMO_PP_PLUGIN_TEXT_DOMAIN)) ?>
        </p>
        <?php
        woocommerce_wp_checkbox( array(
            'label'         => __( 'Apply Global Level Settings' , SUMO_PP_PLUGIN_TEXT_DOMAIN ) ,
    ...
    public static function save_meta( $product_id , $loop = '' , $props = array() ) {
        ...
        foreach( self::$payment_fields as $field_name => $type ) {
            ...
            if( is_numeric( $loop ) ) {
                ...
                if ( 'datepicker-from' === $type && '' !== $posted_meta_data ) {
                    $posted_meta_data .= ' 00:00:00' ;
                }

                if ( 'datepicker-to' === $type && '' !== $posted_meta_data ) {
