...
class SUMO_PP_General_Settings extends SUMO_PP_Abstract_Settings {
    ...
    public function get_settings() {
        ...
        return apply_filters( 'sumopaymentplans_get_' . $this->id . '_settings', array(
            ...
            array(
                'name'     => __( 'Automatic Charge Reminder', $this->text_domain ),
            ...
            array(
                'name'     => __( 'Payment Plans that can be Selected for Multiple Products in a Single Order', $this->text_domain ),
                'id'       => $this->prefix . 'allow_payment_plans_in_cart_as',
                'newids'   => $this->prefix . 'allow_payment_plans_in_cart_as',
                'type'     => 'select',
                'options'  => array(
                    'same-r-different-plan' => __( 'Same/Different', $this->text_domain ),
                    'same-plan'             => __( 'Same', $this->text_domain ),
                ),
            ...
            array(
                'name' => __( 'Overdue Payment Settings', $this->text_domain ),
                'type' => 'title',
            ...
            array(
                'name'    => __( 'Number of Attempts to Charge Automatic Payment during Overdue Status', $this->text_domain ),
                'id'      => $this->prefix . 'automatic_payment_retries_during_overdue',
                'newids'  => $this->prefix . 'automatic_payment_retries_during_overdue',
                'type'    => 'text',
                'std'     => '2',
                'default' => '2',
                'desc'    => __( 'times per day', $this->text_domain ),
            ),
            array( 'type' => 'sectionend', 'id' => $this->prefix . 'overdue_payment_settings' ),
            array(
                'name' => __( 'Payment Gateway Settings', $this->text_domain ),
                'type' => 'title',
            ...
            array(
                'name'    => __( 'Enable Automatic Payment Gateways', $this->text_domain ),
                'id'      => $this->prefix . 'enable_automatic_payment_gateways',
                'newids'  => $this->prefix . 'enable_automatic_payment_gateways',
                'type'    => 'checkbox',
                'std'     => 'yes',
                'default' => 'yes',
                'desc'    => __( 'If enabled, automatic payment gateways will be displayed in checkout page when payment plan/deposit product is added in cart', $this->text_domain ),
            ),
            array(
                'name'    => __( 'Automatic Payment Gateway Mode', $this->text_domain ),
                'id'      => $this->prefix . 'automatic_payment_gateway_mode',
                'newids'  => $this->prefix . 'automatic_payment_gateway_mode',
                'type'    => 'select',
                'options' => array(
                    'auto-or-manual' => __( 'Automatic/Manual', $this->text_domain ),
                    'force-auto'     => __( 'Force Automatic', $this->text_domain ),
                    'force-manual'   => __( 'Force Manual', $this->text_domain ),
                ),
                'std'     => 'auto-or-manual',
                'default' => 'auto-or-manual',
            ),
            array(
                'name'    => __( 'Enable Manual Payment Gateways', $this->text_domain ),
                'id'      => $this->prefix . 'enable_manual_payment_gateways',
                'newids'  => $this->prefix . 'enable_manual_payment_gateways',
                'type'    => 'checkbox',
                'std'     => 'yes',
                'default' => 'yes',
                'desc'    => __( 'If enabled, manual payment gateways will be displayed along with automatic payment gateways in checkout page when payment plan/deposit product is added in cart', $this->text_domain ),
            ),
