...
class SUMO_PP_Admin_Metaboxes {
    ...
    public function render_payment_plan_description( $post ) {
        echo '<p>' ;
        echo '<textarea cols="90" rows="5" name="' . SUMO_PP_PLUGIN_PREFIX . 'plan_description' . '" placeholder="' . __( 'Describe this plan about to customers', SUMO_PP_PLUGIN_TEXT_DOMAIN ) . '">' . get_post_meta( $post->ID, '_plan_description', true ) . '</textarea>' ;
        echo '</p>' ;
    }
    ...
    public function render_payment_orders( $post ) {
        ...
        _sumo_pp_get_payment_orders_table( $post->ID, array(
            'page'        => 'admin',
            'class'       => 'widefat wc_input_table _sumo_pp_footable _sumo_pp_payment-orders',
    ...
    public function save( $post_id, $post, $update ) {
        ...
        switch ( $post->post_type ) {
            ...
            case 'sumo_pp_payments':
                ...
                if ( ! empty( $_POST[ 'expected_payment_date' ] ) ) {
                    if ( 'payment-plans' === $payment->get_payment_type() ) {
                        $existing_scheduled = $new_scheduled      = $payment->get_prop( 'scheduled_payments_date' ) ;

                        if ( is_array( $existing_scheduled ) ) {
                            foreach ( $existing_scheduled as $installment => $schedule ) {
                                if ( isset( $_POST[ 'expected_payment_date' ][ $installment ] ) && ! empty( $_POST[ 'expected_payment_date' ][ $installment ] ) ) {
                                    $new_scheduled[ $installment ] = _sumo_pp_get_timestamp( $_POST[ 'expected_payment_date' ][ $installment ] ) ;
                                }
                            }
                        }
                        ...
                        $new_next_payment_date = $_POST[ 'expected_payment_date' ] ;
