...
class WPWEB_TCPDF {
	 ...
	 */
	protected $cell_height_ratio = WPWEB_K_CELL_HEIGHT_RATIO;
	...
	public function __construct($orientation='P', $unit='mm', $format='A4', $unicode=true, $encoding='UTF-8', $diskcache=false, $pdfa=false) {
		...
		$this->InFooter = false;
		$this->lasth = 0;
		$this->FontFamily = defined('WPWEB_PDF_FONT_NAME_MAIN')?WPWEB_PDF_FONT_NAME_MAIN:'helvetica';
		$this->FontStyle = '';
		$this->FontSizePt = 12;
	...
	public function Error($msg) {
		...
		$this->_destroy(true);
		if (defined('WPWEB_K_TCPDF_THROW_EXCEPTION_ERROR') AND !WPWEB_K_TCPDF_THROW_EXCEPTION_ERROR) {
	...
	public function Close() {
		...
		if ($this->tcpdflink) {
			...
			$font = defined('WPWEB_PDF_FONT_NAME_MAIN')?WPWEB_PDF_FONT_NAME_MAIN:'helvetica';
			$this->SetFont($font, '', 1);
	...
	public function Header() {
		if ($this->header_xobjid === false) {
			...
			if (($headerdata['logo']) AND ($headerdata['logo'] != WPWEB_K_BLANK_IMAGE)) {
		...
		if ($txt != '') {
			...
			if ($this->isunicode) {
				if (($this->CurrentFont['type'] == 'core') OR ($this->CurrentFont['type'] == 'TrueType') OR ($this->CurrentFont['type'] == 'Type1')) {
					...
					if (defined('WPWEB_K_THAI_TOPCHARS') AND (WPWEB_K_THAI_TOPCHARS == true)) {
						// number of chars
				...
				if ($element[0] == '/') {
					...
					if (($dom[$key]['value'] == 'small') OR ($dom[$key]['value'] == 'sup') OR ($dom[$key]['value'] == 'sub')) {
						if (!isset($dom[$key]['attribute']['size']) AND !isset($dom[$key]['style']['font-size'])) {
							$dom[$key]['fontsize'] = $dom[$key]['fontsize'] * WPWEB_K_SMALL_RATIO;
						}
					}
...
Putting 1 is equivalent to putting 0 and calling Ln() just after. Default value: 0.
	...
	protected function openHTMLTagHandler($dom, $key, $cell) {
		...
		switch($tag['value']) {
			...
			case 'tcpdf': {
				if (defined('WPWEB_K_TCPDF_CALLS_IN_HTML') AND (WPWEB_K_TCPDF_CALLS_IN_HTML === true)) {
