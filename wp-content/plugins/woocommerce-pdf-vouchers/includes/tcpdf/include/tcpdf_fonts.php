...
class WPWEB_TCPDF_FONTS {
	...
	public static function _getfontpath() {
		if (!defined('WPWEB_K_PATH_FONTS') AND is_dir($fdir = realpath(dirname(__FILE__).'/../fonts'))) {
			if (substr($fdir, -1) != '/') {
				$fdir .= '/';
			}
			define('WPWEB_K_PATH_FONTS', $fdir);
		}
		return defined('WPWEB_K_PATH_FONTS') ? WPWEB_K_PATH_FONTS : '';
	}
...
// ====================================================================================================================
	...
	public static function UTF8StringToArray($str, $isunicode=true, &$currentfont) {
		...
		if ($isunicode) {
			...
			if( !empty( $chars ) ){
				$carr = array_map(array('WPWEB_TCPDF_FONTS', 'uniord'), $chars);
			}
		} else {
			$chars = str_split($str);
