...
if (! class_exists('Mpbs_Chat_Function_Handler')) {
    /**
     *
     */
    class Mpbs_Chat_Function_Handler implements Util\Admin_Functions_Interface
    {
        ...
        {
            add_action('admin_footer', array($this, 'mpbs_chatbox_template')); 
        ...
        public function mpbs_chatbox_template() {
            ...
            // Admin chatbox
            if ( current_user_can('administrator') && $this->helper->mpbs_author_is_admin($user_id) ) {

                $chatbox = new \WpMarketplaceBuyerSellerChat\Templates\Admin\Mpbs_Admin_Chatbox();
                
                ?>
                <script>
                  window.mpbsSellerChatboxConfig = <?php echo json_encode($this->helper->mpbs_get_admin_chatbox_config()); ?>;
                  
                </script>
                <?php
                echo '<div class="mpbs-active-users">';
                    $chatbox->mpbs_admin_chatbox();
                echo '</div>';
