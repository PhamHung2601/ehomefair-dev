 ...
 * 				)
 */
function paginate_form( $formsetting, $field_defs, $form_values = '' ) {
	
	$formsetting_raw = $formsetting;
	$forms_view = $formsetting_raw['fields'];
	$forms_pref = $formsetting_raw['fieldsinfo'];
	$formfields = new mpcustomformfields();
	$advancedfields = new mpcustomadvance();
	$commonfields = $field_defs['comman_fields'];
	$advanced_fields = $field_defs['adavanced_fields'];
	...
	foreach ($forms_view as $id => $type) {
		...
		if ($type == 'Pageseparator') {
			...
			$tmp_obj = new $type();
			$part_name = $tmp_obj->field_render_html( $cur_pref, $form_values );
		...
		$tmp_obj = new $type();
		$part_html .= $tmp_obj->field_render_html( $cur_pref, $form_values );
	}
	if (!empty($part_html)) {
		$prev_part = $part_number - 1;
		$parent_part = $part_number;
		...
		$form_parts_html[] = $part_html . "</div>"; // @todo: Final submit button html has to be added
	}
