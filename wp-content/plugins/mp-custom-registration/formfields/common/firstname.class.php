...
class Firstname {
	...
	public function control_button() {
		ob_start();
		?>
		<li class="list-group-item" data-type="<?php echo __CLASS__ ?>" for="firstname">
			<span class="lfi lfi-name"></span>First Name
			<a title="First Name" rel="Firstname" class="add" data-template='Name' href="#"><i class="glyphicon glyphicon-plus-sign pull-right ttipf" title=""></i></a>
		</li>
	    <?php
...
?>
				...
				<fieldset>
					...
					<div class="form-group">
						<label><input rel="req-params" class="req" type="checkbox"
									  ...
									  value="1" <?php echo (isset($field_infos[$fieldindex]['required']) ? "checked=checked" : "") ?> />Required</label>
	...
	public function field_render_html( $params = array(), $form_val = [] ) {
		...
		<div id="<?php echo $params['id'] ?>" class='form-group <?php if (isset($params['conditioned'])) echo " conditioned hide "?>' data-cond-fields="<?php echo $condition_fields ?>" data-cond-action="<?php echo $cond_action.':'.$cond_boolean ?>" >
			...
			<input type='text'  name='submitform[<?php echo isset($params['id'])?$params['id']:'' ?>]'  value='<?php echo $value_field; ?>' <?php echo mpCustomRequired($params)?> class='form-control ' />
		</div>
		<?php
		$field_render_html = ob_get_clean();
		return $field_render_html;
