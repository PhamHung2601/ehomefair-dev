...
class Radio {
	...
	public function control_button() {
		...
		<li class="list-group-item" data-type="<?php echo __CLASS__ ?>" for="Radio">
			<span class="lfi lfi-name"></span> Radio
			<a title="Radio" rel="Radio" class="add" data-template='Radio' href="#"><i class="glyphicon glyphicon-plus-sign pull-right ttipf" title=""></i></a>
		</li>
	    <?php
...
?>
	...
	public function field_render_html($params = array(), $form_val = [] ) {
		...
		<div id="<?php echo $params['id'] ?>" class='form-group <?php if (isset($params['conditioned'])) echo " conditioned hide "?>' data-cond-fields="<?php echo $condition_fields ?>" data-cond-action="<?php echo $cond_action.':'.$cond_boolean ?>" >
			...
			?>
				<label><input type='radio' value='<?php echo $option_values[$id] ?>' <?php echo $checked; ?> name='submitform[<?php echo $params['id'] ?>]' <?php echo mpCustomRequired($params) ?> /> <?php echo $label ?></label>
			<?php
			}
			?>
			</div>
