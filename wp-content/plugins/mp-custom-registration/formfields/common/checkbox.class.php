...
class Checkbox {
	public function control_button() {
		...
		<li class="list-group-item" data-type="<?php echo __CLASS__ ?>" for="Checkbox">
			<span class="lfi lfi-name"></span> Checkbox
			<a title="Checkbox" rel="Checkbox" class="add" data-template='Checkbox' href="#"><i class="glyphicon glyphicon-plus-sign pull-right ttipf" title=""></i></a>
		</li>
	    <?php
...
?>
	...
	public function field_preview_html($params = array()) {
		...
		?>
			<label><input type='checkbox' disabled="disabled" value='<?php echo $option_values[$id] ?>' name='submitform[<?php echo $params['id'] ?>][]' <?php echo mpCustomRequired($params) ?>   /><?php echo $label ?></label>
		<?php
        }
		?>
        </div>
	...
	public function field_render_html($params = array(), $form_val = [] ) {
		...
		if ( ! empty( $form_val ) && is_array( $form_val ) && isset( $form_val[$params['id']] ) ) {
		...
		<div id="<?php echo $params['id'] ?>" class='form-group <?php if (isset($params['conditioned'])) echo " conditioned hide "?>' data-cond-fields="<?php echo $condition_fields ?>" data-cond-action="<?php echo $cond_action.':'.$cond_boolean ?>" >
			...
			<?php
			foreach ($option_names as $id => $label) {
				if ( ( is_array( $value_field ) && isset( $value_field[$id] ) && $option_values[$id] === $value_field[$id] ) || $option_values[$id] === $value_field ){
					$checked = 'checked';
				} else {
					$checked = '';
				}
			?>
				<label><input type='checkbox' value='<?php echo $option_values[$id] ?>' <?php echo $checked; ?>  name='submitform[<?php echo $params['id'] ?>][]' <?php echo mpCustomRequired($params) ?> /><?php echo $label ?></label>
			<?php
			}
			?>
			</div>
