...
class Select {
	...
	public function control_button() {
		...
		<li class="list-group-item" data-type="<?php echo __CLASS__ ?>" for="Select">
			<span class="lfi lfi-name"></span> Select
			<a title="Select" rel="Select" class="add" data-template='Select' href="#"><i class="glyphicon glyphicon-plus-sign pull-right ttipf" title=""></i></a>
		</li>
	    <?php
...
?>
					...
					<div class="form-group">
						...
						<div
							...
							<select
								...
								class="form-control">
									<?php
									$validation_ops = getCustomValidation();
	...
	public function field_preview_html($params = array()) {
		...
		ob_start();
		?>
		<select disabled="disabled" name='submitform[]' <?php echo isset($params['multi'])?'multiple':'' ?> <?php echo mpCustomRequired($params) ?>>
	...
	public function field_render_html($params = array(), $form_val = [] ) {
        $option_names = isset( $params['options']['name'] ) && $params['options']['name'] ? $params['options']['name'] : array();
		$option_names = $option_names && is_array( $option_names ) ? array_filter( $option_names ) : [];

		$option_values = isset( $params['options']['value'] ) && $params['options']['value'] ? $params['options']['value'] : array();
		$option_values = $option_values && is_array( $option_values ) ? array_filter( $option_values ) : [];
		...
		<div id="<?php echo $params['id'] ?>" class='form-group <?php if (isset($params['conditioned'])) echo " conditioned hide "?>' data-cond-fields="<?php echo $condition_fields ?>" data-cond-action="<?php echo $cond_action.':'.$cond_boolean ?>" >
			...
			<select name='submitform[<?php echo $params['id'] ?>]' <?php echo isset($params['multi'])?'multiple':'' ?> <?php echo mpCustomRequired($params) ?>  class=''>
				<option selected='selected' value=''>Please Select</option>
			<?php
			foreach ($option_names as $id => $label) {
				if ( $option_values[$id] === $value_field ){
					$checked = 'selected';
				} else {
					$checked = '';
				}
				?>
				<option value='<?php echo $option_values[$id]; ?>'  <?php echo $checked; ?>><?php echo $label ?></option>
			<?php
			}
			?>
			</select>
