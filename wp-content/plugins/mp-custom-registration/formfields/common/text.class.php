...
class Text {
	...
	public function control_button() {
		...
		<li class="list-group-item" data-type="<?php echo __CLASS__ ?>" for="Text">
			<span class="lfi lfi-name"></span> Text
			<a title="Text" rel="Text" class="add" data-template='Text' href="#"><i class="glyphicon glyphicon-plus-sign pull-right ttipf" title=""></i></a>
	</li>
	<?php
...
?>
	...
	public function field_render_html( $params = array(), $form_val = [] ) {
		...
		<div id="<?php echo $params['id'] ?>" class='form-group <?php if (isset($params['conditioned'])) echo " conditioned hide "?>' data-cond-fields="<?php echo $condition_fields ?>" data-cond-action="<?php echo $cond_action.':'.$cond_boolean ?>" >
			...
			<input type='text'  name='submitform[<?php echo isset($params['id'])?$params['id']:'' ?>]' class='form-control' value='<?php echo $value_field?>' <?php echo mpCustomRequired($params)?> />
		</div>
		<?php
		$field_render_html = ob_get_clean();
		return $field_render_html;
