...
class FullName {
	...
	public function control_button() {
		...
		<li class="list-group-item" data-type="<?php echo __CLASS__ ?>" for="FullName">
			<span class="lfi lfi-name"></span> Full Name
			<a title="FullName" rel="FullName" class="add" data-template='FullName' href="#"><i class="glyphicon glyphicon-plus-sign pull-right ttipf" title=""></i></a>
	</li>
	    <?php
		...
		<div id="<?php echo $params['id'] ?>" class='form-group <?php if (isset($params['conditioned'])) echo " conditioned hide "?>' data-cond-fields="<?php echo $condition_fields ?>" data-cond-action="<?php echo $cond_action.':'.$cond_boolean ?>" >
			...
			<div class='form-group' >
				<div class='row'>
					<div class='col-md-2'>
						<label for='title'>Title: </label>
						<input class='form-control' type='text' value='<?php if( $value_field != '' && $value_field['title'] != '' ) echo $value_field['title'];?>' name='submitform[<?php echo $params['id'] ?>][title]' id='title' <?php echo mpCustomRequired($params) ?> />
					</div>
					<div class="col-md-5">
						<label for='first-name'>First name: </label>
						<input class='form-control' type='text' value='<?php if( $value_field != '' && $value_field['first_name'] != '' ) echo $value_field['first_name'];?>' name='submitform[<?php echo $params['id'] ?>][first_name]' id='first_name' <?php echo mpCustomRequired($params) ?> />
					</div>
					<div class="col-md-5">
						<label for='last-name'>Last name: </label>
						<input class='form-control' type='text' value='<?php if( $value_field != '' && $value_field['last_name'] != '' ) echo $value_field['last_name'];?>' name='submitform[<?php echo $params['id'] ?>][last_name]' id='last_name' <?php echo mpCustomRequired($params) ?> />
					</div>
				</div>
			</div>
		</div>
