...
class Daterange {
	...
	public function control_button() {
		...
		<li class="list-group-item" data-type="<?php echo __CLASS__ ?>" for="Daterange">
			<span class="lfi lfi-name"></span> Date range
			<a title="Daterange" rel="Daterange" class="add" data-template='Daterange' href="#"><i class="glyphicon glyphicon-plus-sign pull-right ttipf" title=""></i></a>
		</li>
	    <?php
...
?>
	...
	public function field_render_html($params = array(), $form_val = [] ) {
		...
		if ( ! empty( $form_val ) && is_array( $form_val ) && array_key_exists( $params['id'].'_1', $form_val ) && array_key_exists( $params['id'].'_2', $form_val ) ) {
			$value_field_1 = $form_val[$params['id'].'_1'];
			$value_field_2 = $form_val[$params['id'].'_2'];
		} else {
			$value_field_1 = '';
			$value_field_2 = '';
		}
		...
		<div id="<?php echo $params['id'] ?>" class='form-group <?php if (isset($params['conditioned'])) echo " conditioned hide "?>' data-cond-fields="<?php echo $condition_fields ?>" data-cond-action="<?php echo $cond_action.':'.$cond_boolean ?>" >
			...
			<div class='row' >
				<div class="col-md-6">
					<input class='form-control datepicker' type='text' value = '<?php echo $value_field_1?>' name='submitform[<?php echo $params['id'].'_1' ?>]' id='<?php echo $datepicker_field1_id ?>' <?php echo mpCustomRequired($params) ?>/>
				</div>
				<div class="col-md-6">
					<input class='form-control datepicker' type='text' value = '<?php echo $value_field_2?>' name='submitform[<?php echo $params['id'].'_2' ?>]' id='<?php echo $datepicker_field2_id ?>' <?php echo mpCustomRequired($params) ?>/>
				</div>
			</div>
		</div>
