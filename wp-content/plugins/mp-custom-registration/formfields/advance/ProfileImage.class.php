...
class ProfileImage {
	...
	public function control_button() {
		ob_start();
		?>
		<li class="list-group-item" data-type="<?php echo __CLASS__ ?>" for="ProfileImage">
			<span class="lfi lfi-name"></span> Profile Image
			<a title="Profile Image" rel="ProfileImage" class="add" data-template='File' href="#"><i class="glyphicon glyphicon-plus-sign pull-right ttipf" title=""></i></a>
		</li>
		<?php
		...
		<div id="<?php echo $params['id'] ?>" class='form-group <?php if (isset($params['conditioned'])) echo " conditioned hide "?>' data-cond-fields="<?php echo $condition_fields ?>" data-cond-action="<?php echo $cond_action.':'.$cond_boolean ?>" >
			...
			<?php 
				if(get_current_user_id()){
					...
					if(isset(get_user_meta($user_id,'mp_custom_data', true)['image'])){
						$uploaded_pro_img = get_user_meta(get_current_user_id(),'mp_custom_data', true)['image'];
					?>
						<img src="<?php echo $uploaded_pro_img; ?>" width="150" height="auto">
					<?php
					}
					?>
					<input type='file' name='submitform[<?php echo $params['id'] ?>]' <?php echo mpCustomRequired($params) ?>/>
					<?php
				}
				else{
				?>
	...
	public function configuration_template() {
		...
		<script type="text/x-mustache" id="template-File">
			<li class="list-group-item" data-type="<?php echo __CLASS__ ?>" id="field_{{ID}}">
			<input type="hidden" name="contact[fields][{{ID}}]" value="{{value}}">
