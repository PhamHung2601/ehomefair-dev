...
class Url {
	...
	public function control_button() {
		...
		<li class="list-group-item" data-type="<?php echo __CLASS__ ?>" for="Url">
			<span class="lfi lfi-name"></span> URL
			<a title="Url" rel="Url" class="add" data-template='Url' href="#"><i class="glyphicon glyphicon-plus-sign pull-right ttipf" title=""></i></a>
		</li>
	    <?php
...
?>
	...
	public function field_render_html($params = array(), $form_val = [] ) {
		...
		<div id="<?php echo $params['id'] ?>" class='form-group <?php if (isset($params['conditioned'])) echo " conditioned hide "?>' data-cond-fields="<?php echo $condition_fields ?>" data-cond-action="<?php echo $cond_action.':'.$cond_boolean ?>" >
			...
			<div class='form-group'>
				<input type='text' class='form-control' name=submitform[<?php echo $params['id'] ?>] id='urlfield_<?php echo $params['id'] ?>' value='<?php echo $value_field; ?>' <?php echo mpCustomRequired($params) ?> placeholder="http://www.website.com" '/>
			</div>
		</div>
		<?php
		$field_render_html = ob_get_clean();
