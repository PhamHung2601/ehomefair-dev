...
if ( is_array( $data ) ) {
	foreach ( $data as $key => $value ) {
        $order = new WC_Order( $value->get_order_id() );
...
$seller_order_refund_data = $order_refund->wkmp_get_seller_order_refund_data( $order->get_id() );
...
if( !empty( $refunded_amount ) ) {
    $refund_statement = '<p>' . utf8_decode( sprintf( esc_html__( 'Your order %s has been refunded by amount %s. There are more details below for your reference:', 'marketplace' ), esc_html( $order->get_order_number() ), wc_price( $refunded_amount, array( 'currency' => $order->get_currency() ) ) ) ) . '</p>';
} else {
    $refund_statement = '<p>' . utf8_decode( sprintf( esc_html__( 'Your order %s has been fully refunded. There are more details below for your reference:', 'marketplace' ), esc_html( $order->get_order_number() ) ) ) . '</p>';
}
...
$result = '
	<div style="margin-bottom: 40px;">
		...
		<table class="td" cellspacing="0" cellpadding="6" style="width: 100%; font-family: "Helvetica Neue", Helvetica, Roboto, Arial, sans-serif;" border="1">
			...
			$reward_used = get_post_meta( $order->get_id(), '_wkmpreward_points_used', true );
			...
			if( !empty( $reward_used ) ) {
				...
				$result .=
					'<tr>
						<th class="td" scope="row" colspan="2" style="text-align:' . esc_attr( $text_align ) . ';">' . utf8_decode( __( 'Reward Points', 'marketplace' ) ) . ' : </th>
						<td class="td">' . wc_price( -$reward_used, array( 'currency' => $order->get_currency() ) ) . '</td>
					</tr>';
			}
			...
			$result .=
                ...
                if( !empty( $seller_order_refund_data[ 'refunded_amount' ] ) ) {
                    ...
                    $result .= '<td class="td"><strong><del>' . wc_price( $total_payment, array( 'currency' => $order->get_currency() ) ) . '</del></strong> ' . wc_price( $total_payment - $seller_order_refund_data[ 'refunded_amount' ], array( 'currency' => $order->get_currency() ) ) . '</td>';

                } else {

                    $result .= '<td class="td">' . wc_price( $total_payment, array( 'currency' => $order->get_currency() ) ) . '</td>';
            ...
            if( !empty( $seller_order_refund_data[ 'refunded_amount' ] ) ) {
                ...
                $result .=
					'<tr>
						<th class="td" scope="row" colspan="2" style="text-align:' . esc_attr( $text_align ) . ';">' . utf8_decode( esc_html__( 'Refunded', 'marketplace' ) ) . ' : </th>
						<td class="td">' . wc_price( $seller_order_refund_data[ 'refunded_amount' ], array( 'currency' => $order->get_currency() ) ) . '</td>
					</tr>';
